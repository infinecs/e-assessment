    ...
    <div class="col-span-12 xl:col-span-6">
        <div class="card dark:bg-zinc-800 dark:border-zinc-600">
            ...
            <div class="card-body border-b border-gray-100 dark:border-zinc-600 flex items-center justify-between">
                <h6 class="mb-1 text-gray-700 text-15 dark:text-gray-100">Assessment Events</h6>
            ...
            <div class="card-body">
                <div class="relative rounded-lg" style="max-height: 500px; overflow-y: auto; overflow-x: auto;">
                    <table class="w-full min-w-[1100px] text-xs text-center text-gray-500">
                        <thead
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Event Name</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Event Code</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Actions</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Question Limit</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Duration Each Question</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">Start Date</th>
                                <th class="px-6 py-3 bg-gray-50 dark:bg-zinc-700">End Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($records as $row)
                                <tr
                                    ...
                                    <td class="px-6 py-4">{{ $row->EventName }}</td>
                                    <td class="px-6 py-4">{{ $row->EventCode }}</td>
                                    ...
                                    <td class="px-5 py-4">{{ $row->QuestionLimit }}</td>
                                    <td class="px-5 py-4">{{ $row->DurationEachQuestion }}</td>
                                    <td class="px-7 py-4">{{ \Carbon\Carbon::parse($row->StartDate)->format('d M Y') }}</td>
                                    <td class="px-7 py-4">{{ \Carbon\Carbon::parse($row->EndDate)->format('d M Y') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="12" class="px-6 py-4 text-center">No events found</td>
                                </tr>
                            @endforelse
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.dropdown').forEach(dropdown => {
            ...
            toggle.addEventListener('click', (e) => {
                e.stopPropagation();
                document.querySelectorAll('.dropdown-menu').forEach(m => m.classList.add(
                    'hidden'));
